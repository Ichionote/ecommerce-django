...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = "Por favor activa tu uenta en la pagina"
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Felicidade, tu cuenta esta activa')
        return redirect('login')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user,token):
        ...
        messages.success(request, 'Por favor restea tu password')
        return redirect('resetPassword')
    
    else:
        messages.error(request,'El link ha expiradp')
        return redirect('login')
